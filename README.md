...
1.5초 음성 파일 하나를 판단하는 데 총 **21.30ms** 소요.

| 단계 | 시간 | 설명 |
|------|------|------|
| LogMel 특징 추출 (CPU) | ~13ms | 음성 → 스펙트로그램 변환 |
| NPU 추론 | ~8ms | 스펙트로그램 → 웨이크워드 확률 |
| **합계** | **~21ms** | |

실시간 스트리밍에서는 160ms마다 한 번 판단하므로, 판단 한 번에 21ms면 NPU 사용률 약 13%로 매우 여유롭다.

### 정확도 상세

1,897개 테스트 음성으로 평가한 결과:

| | 모델이 "아니다" 판정 | 모델이 "맞다" 판정 |
|---|---|---|
| **실제로 웨이크워드 아닌 파일 (1,616개)** | 1,608개 ✅ 정확 | 8개 ❌ 오탐 |
| **실제로 웨이크워드인 파일 (281개)** | 17개 ❌ 미탐 | 264개 ✅ 정확 |

- **오탐**: "하이 원더" 안 말했는데 반응 → 1,616개 중 8개 (0.50%) → **약 200번에 1번**
- **미탐**: "하이 원더" 말했는데 못 알아들음 → 281개 중 17개 (6.05%) → **약 17번에 1번**
- **FAR (오경보율)**: 118분 TV 뉴스 배경음 기준 **0.00회/시간** (threshold=0.55 + EMA)
